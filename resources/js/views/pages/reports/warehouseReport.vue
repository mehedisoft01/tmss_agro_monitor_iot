<script setup>
    ...
    const excelUrl = () => {
        ...
        window.open(`/api/warehouse_reports/excel?${params}`, "_blank");
    };

    onMounted(() => {
        getDependency({ dependency: ["warehouse_device"] });
    });
</script>

<template>
    <reportComponent :headings="['Sl','Device Name','Date','Temperature(°C)','Humidity(%)','Remarks']" :setting="true">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <select class="form-control" v-model="formFilterState.device_id">
                                ...
                                <option v-for="data in pageDependencies.warehouse_device || []" :key="data.device_id" :value="data.device_id">
                                    {{ data.display_name }}
                                </option>
                            </select>
                        </div>
